...
class FrameLayout(wx.Frame):
    def __init__(self, parent):
        ...
        self.title = wx.StaticText(self.panel, label='Esper Device Actions at the Group Level')

        if (self.WINDOWS):
            font = wx.Font(10, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_LIGHT)
        else:
        ...
        self.configList = wx.TextCtrl(self.panel, style = wx.TE_MULTILINE | wx.TE_READONLY, size=(400,80))
        ...
        # Actions UI
        self.groupLabel = wx.StaticText(self.panel, label='Choose the Group to take Action on')
        self.groupChoice = wx.ComboBox(self.panel, wx.CB_READONLY, size=(100,-1))
        self.actionLabel = wx.StaticText(self.panel, label='Action to apply to Devices in Group')
        self.actionChoice = wx.Choice(self.panel, wx.CB_SORT, size=(100,-1), choices=Globals.ACTIONS)
    ...
    def onGoButton(self, event):
        ...
        elif (self.actionChoice.GetSelection() == Globals.SET_KIOSK and self.appChoice.GetSelection() < 0):
            wx.MessageBox("Please select an Application", style=wx.OK)
        else:
            groupLabel = self.groupChoice.Items[self.groupChoice.GetSelection()]
            TakeAction(self, self.groupChoice.GetSelection(), self.actionChoice.GetSelection(), groupLabel)
    ...
    def SetConfigValues(self):
        ...
        if ('https' in str(selectedConfig['apiHost'])): 
            ...
            Globals.configuration.api_key_prefix['Authorization'] = selectedConfig['apiPrefix']
    ...
    def PopulateGroups(self):
        ...
        try:
            api_response = api_instance.get_all_groups(Globals.enterprise_id,limit=limit,offset=offset)
    ...
    def PopulateApps(self):
        """create an instance of the API class"""
        api_instance = esperclient.ApplicationApi(esperclient.ApiClient(Globals.configuration))
        limit = 5000 # int | Number of results to return per page. (optional) (default to 20)
        offset = 0 # int | The initial index from which to return the results. (optional) (default to 0)
        self.appChoice.Clear()
        try:
            api_response = api_instance.get_all_applications(Globals.enterprise_id,limit=limit,offset=offset, is_hidden=False)
            if len(api_response.results):
    ...
    def LoadTagsAndAliases(self):
        ...
        if (os.path.isfile(namestagsfile)):
            ...
            self.Logging('--->****' + ' Tag/Name CSV ' + ' not found - Set Names/Tags Action will not work')
    ...
    def PopulateConfig(self):
        ...
        if (os.path.isfile(configfile)):
            ...
            self.Logging('--->****' + Globals.CONFIGFILE + ' not found - PLEASE Quit and create configuration file')  
    ...
    def SaveLogging(self):
        ...
        self.Logging("---> Logging info saved to csv file - " + Globals.csv_tag_path_clone)

    def CreateNewFile(self):
        newFile = 'output.csv'
        Globals.csv_tag_path_clone = newFile
        if not os.path.exists(Globals.csv_tag_path_clone):
            with open(Globals.csv_tag_path_clone, 'w'): pass
    ...
    def createLogString(self, deviceInfo, action):
        ...
        if (action == Globals.SHOW_DEVICES):
            ...
            if ('KioskApp' in deviceInfo):
                logString = logString + ',' + \
                                    '{:16.16}'.format(deviceInfo['KioskApp'])
        return logString

    def askForNameTagCSV(self):
        #Windows, Standalone executable will allow user to select CSV
        if 'Windows' in platform.system():
            answer = ctypes.windll.user32.MessageBoxW(0, "Please Select The Device List CSV", "Esper Tool", 1)                
            if answer != 2:
                root = Tk()
        ...
        #Mac, Debug mode, find csv file using system path
            ...
            currentpath = os.path.realpath(__file__)
            filename = os.path.dirname(currentpath) + os.path.sep + Globals.NAMESTAGSFILE
            Globals.csv_tag_path = filename
