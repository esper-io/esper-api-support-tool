...
def getInfo(request_extension, deviceid):
    ...
    headers = { "Authorization" : f"Bearer {Globals.configuration.api_key['Authorization']}", 'Content-Type': "application/json",}
    url = Globals.BASE_REQUEST_URL.format(configuration_host=Globals.configuration.host, enterprise_id=Globals.enterprise_id, device_id=deviceid) + request_extension
    resp = requests.get(url, headers=headers)
    json_resp = resp.json()
    print("Response {result}".format(result=json.dumps(json_resp, indent=4, sort_keys=True)))
    return json_resp
...
def toggleKioskMode(frame, deviceid, appToUse, isKiosk):
    ...
    api_instance = esperclient.CommandsApi(esperclient.ApiClient(Globals.configuration))
    if isKiosk:
        command_args = esperclient.CommandArgs(package_name=appToUse)  
    else:
        command_args = {}
    command = esperclient.CommandRequest(command='SET_KIOSK_APP', command_args=command_args)    
    api_response = api_instance.run_command(Globals.enterprise_id, deviceid, command)
    response  = api_instance.get_command(api_response.id, deviceid, Globals.enterprise_id)
    status = response.state
    frame.Logging("---> " + str(status))
    
    while status not in ["Command Success", "Command Failure", "Command TimeOut", "Command Cancelled"]:
        time.sleep(1)
        response  = api_instance.get_command(api_response.id, deviceid, Globals.enterprise_id)
        status = response.state
        frame.Logging("---> " + str(status))
...
def getdeviceapps(deviceid):
    ...
    if len(json_resp['results']):
        for app in json_resp['results']:
            applist.append(app['application']['application_name'] + " v" + app['application']['version']['version_code'])
    return applist
...
def setdevicename(frame, deviceid, devicename):
    ...
    api_instance = esperclient.CommandsV2Api(esperclient.ApiClient(Globals.configuration))
    args = esperclient.V0CommandArgs(device_alias_name = devicename)  
    command = esperclient.V0CommandRequest(command_type='DEVICE', devices=[deviceid], command='UPDATE_DEVICE_CONFIG',
                                                   command_args=args, device_type='all')    
    api_response = api_instance.create_command(Globals.enterprise_id, command)
    response = api_instance.get_command_request_status(Globals.enterprise_id, api_response.id)
...
def iterateThroughDeviceList(frame, action, api_response):
    ...
    for device in api_response.results:
        ...
        if (action == Globals.SHOW_DEVICES or action == Globals.SHOW_APP_VERSION or action == Globals.SHOW_ALL or action == Globals.SHOW_BLUETOOTH):
...
def TakeAction(frame, group, action, label):
    ...
    frame.Logging("---> Starting Execution - " + str(Globals.ACTIONS[action]) + " on " + frame.groupChoice.GetString(group))
    if (action == Globals.SHOW_NamesAndTags):
        ...
        for serial, names in Globals.TAGSandALIASES.items():
            tag = names[1]
            frame.Logging(', , ' + '{:18.16}'.format(names[0]) + ', , , ' + '{:18.16}'.format(serial) + ', ' + tag)
    else:
...
def setKiosk(frame, device, deviceInfo):
    ...
    if (deviceInfo['Mode'] == 'Multi'):
        if (deviceInfo['Status'] == "On-Line"):
            ...
            frame.Logging(str("--->" + str(device.device_name) + " " + str(device.alias_name)) + " " + ",->Kiosk->" + str(appToUse))
...
def setMulti(frame, device, deviceInfo):
    ...
    if (deviceInfo['Mode'] == 'Kiosk'):
        if (deviceInfo['Status'] == "On-Line"):
            ...
            frame.Logging(str("--->" + str(device.device_name) + " " + str(device.alias_name)) + " " + ",->Multi->")
    ...
    if serialNum in Globals.TAGSandALIASES.keys():
        ...
        if (not(newName in str(device.alias_name))):
            if (deviceInfo['Status'] == "On-Line"):
                ...
                frame.Logging(str("--->" + str(device.device_name) + " " + str(device.alias_name)) + " " + ",->Name->" + newName)
